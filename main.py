...
word_temp = "{input}에 관한 시를 써줘.\작성한 시를 10점 만점으로 평가해.\만약 시가 8점을 넘지 못했다면 다시 생성해.\8점이 넘었다면 시 생성은 멈추고 8점이 넘은 시를 결과로 해.\json 형식으로 시도 횟수(attempts), 제목(head), 내용(body), 점수(rating)와 간단한 분석(reason)을 출력해.\json으로 파싱할 수 있어야 하므로 json 앞에 아무 단어도 있어선 안돼.\\head의 크기와 색상은 markdown을 활용해 시에 알맞게 조절해.\body도 크기와 색상을 markdown을 활용해 조정하는데 각 단어별로 다르게 해도 돼.\시에서 줄바꿈은 <br>로 처리해.\프롬프트에 적힌 내용을 echo하지 말것.\제목과 내용의 키는 각각 head, body여야만 해.\시는 다른 언어일 수 있지만 reason은 항상 한국어여야만 해."
sentence_temp = "{input}에서 시의 주제를 추출해.\주제를 글 내에서 자유롭게 추출해도 되지만 전체 내용와 연관이 있기는 해야돼.\추출한 주제에 관한 시를 하나 써줘.\작성한 시를 10점 만점으로 평가해.\만약 시가 8점을 넘지 못했다면 다시 생성해.\8점이 넘었다면 시 생성은 멈추고 8점이 넘은 시를 결과로 해.\json 형식으로 시도 횟수(attempts), 제목(head), 내용(body), 점수(rating)와 간단한 분석(reason)을 출력해.\json으로 파싱할 수 있어야 하므로 json 앞에 아무 단어도 있어선 안돼.\\head의 크기와 색상은 markdown을 활용해 시에 알맞게 조절해.\body도 크기와 색상을 markdown을 활용해 조정하는데 각 단어별로 다르게 해도 돼.\시에서 줄바꿈은 <br>로 처리해.\프롬프트에 적힌 내용을 echo하지 말것.\제목과 내용의 키는 각각 head, body여야만 해.\시는 다른 언어일 수 있지만 reason은 항상 한국어여야만 해."
subRoute = [
        {
            "name": "word",
